...
try:
    ...
    zstd = None
    print('[WARN] zstandard not installed — checker limited')

try:
    from Crypto.Cipher import AES
except ImportError:
    AES = None
    print('[WARN] pycryptodome not installed — checker limited')
...
try:
    ...
    HAS_RICH = False
    # minimal stubs so import never kills the web app
    class _Dummy:
    ...
    Console = RichGroup = Live = Table = Panel = Text = Align = _Dummy
    DOUBLE = HEAVY = ROUNDED = SIMPLE_HEAVY = MINIMAL = MINIMAL_HEAVY_HEAD = None
    Theme = Style = Layout = Columns = Padding = Progress = _Dummy
    BarColumn = TextColumn = TimeElapsedColumn = SpinnerColumn = _Dummy
    def escape(x): return str(x)
    print('[WARN] rich not installed — CLI UI disabled, checker API still works')
